...
io.on('connect', (socket) => {
  ...
  socket.on('set', function(field, value, time, callback){
    if (field in db) {
      ...
      // opération processeur -> 1ns // un evoi de fichier volumineux de l'ordre de ms.
      ...
      // Si timestamp identique -- comparer les valeurs si identique - fixer
      // une règle de comparaison sur les hashs. 
      all_keys[field] = time;
      setTimeout( () => {
        config["friends"].map(
          port => {
